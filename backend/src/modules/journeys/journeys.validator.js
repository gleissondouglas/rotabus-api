 ...
 */
const planJourneySchema = z.preprocess((data) => {
  ...
  origin: z.preprocess((val) => {
    ...
    }).min(-90, "A latitude da origem deve estar entre -90 e 90.").max(90, "A latitude da origem deve estar entre -90 e 90."),
    lng: z.number({
      invalid_type_error: "Latitude e longitude da origem devem ser números.",
      required_error: "Latitude e longitude da origem devem ser números."
    }).min(-180, "A longitude da origem deve estar entre -180 e 180.").max(180, "A longitude da origem deve estar entre -180 e 180.")
  })),
  destination: z.preprocess((val) => {
    if (val && typeof val === 'object' && !Array.isArray(val)) return val;
    ...
    text: z.string({
      ...
      .refine(t => t.length >= 2, "O texto do destino deve ter pelo menos 2 caracteres.")
      .refine(t => t.length <= 200, "O texto do destino é muito longo."),
    lat: z.number().min(-90, "A latitude do destino é inválida.").max(90, "A latitude do destino é inválida.").optional(),
    lng: z.number().min(-180, "A longitude do destino é inválida.").max(180, "A longitude do destino é inválida.").optional()
  })),
  timePreference: z.preprocess((val) => {
  ...
  }, timePreferenceSchema).optional(),
  departureTime: z.string().refine((val) => !Number.isNaN(new Date(val).getTime()), {
    message: "O horário de partida deve ser uma data válida."
  }).optional(),
  routingPreference: z.preprocess((val) => {
    const validRoutingPreferences = ["ANY_ROUTE", "LESS_WALKING", "FEWER_TRANSFERS"];
    if (val && typeof val === "string" && validRoutingPreferences.includes(val.toUpperCase())) {
 ...
 */
const resolveDestinationSchema = z.preprocess((data) => {
  if (!data || typeof data !== 'object') return data;
